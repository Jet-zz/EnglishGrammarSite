      ...
      <section className="mt-6 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 flex gap-6 text-sm">
          <div className="flex items-center gap-2">
            ...
            <span className="text-slate-600">主干：句子骨架</span>
          </div>
          <div className="flex items-center gap-2">
            <span className="inline-block h-3 w-3 rounded bg-blue-200" />
            <span className="text-slate-600">修饰：让信息更具体</span>
          </div>
        </div>
        ...
        <section className="rounded-xl border border-blue-200 bg-blue-50 p-5 shadow-sm">
          ...
          <div className="mt-4 grid gap-2">
            {overviewData.modifiers.map((m) => (
              <div key={m.name} className="rounded-lg border border-blue-200 bg-white px-4 py-3">
                <span className="font-semibold text-blue-900">{m.name}</span>
                <span className="ml-3 text-sm text-slate-600">{m.description}</span>
              </div>
            ))}
